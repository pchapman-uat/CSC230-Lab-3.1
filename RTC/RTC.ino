...
void setup() {
    M5.begin();  
    M5.Axp.ScreenBreath(50);  // Init M5StickC Plus.  初始化 M5StickC Plus
    ...
    // Following lines where adjusted to todays time

    TimeStruct.Hours   = 19;
    TimeStruct.Minutes = 33;
    TimeStruct.Seconds = 0;
    M5.Rtc.SetTime(&TimeStruct);  // writes the set time to the real time clock.
                                  // 将设置的时间写入实时时钟
    RTC_DateTypeDef DateStruct;

    // Following lines where adjusted to todays date
